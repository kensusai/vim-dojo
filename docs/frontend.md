...
- **計測結果(2026-07-11、本番 GitHub Pages・headless Chromium・高速回線)**: 実行は `node e2e/perf-probe.mjs`。
  - 初回ロード: TTFB 227ms / FCP 528ms / LCP 560ms / 操作可能 554ms(目標 LCP 2.5s に対し4倍以上の余裕)
  - 再訪(Service Worker キャッシュ): リロード→操作可能 **60ms**(「再訪時は即表示」達成)
  - タイピング連打: 52キーで long task(>50ms)ゼロ(キー入力16ms要件達成)
  - バンドル: 単一チャンク 840KB(gzip 265KB)+ CSS 5.6KB gzip
- **見直し条件(この数値を切ったら対処)**: 実回線での cold LCP が 1.5s を超える、または gzip バンドルが 400KB を超えたら、Motion・分析系の遅延ロードでコード分割する。それまでは分割しない(計測なしに最適化しない)。
- **弱点分析のスケール**: ホーム訪問時の attempts 全件読込+分析は 20,000件(数年分)で parse 24ms + 集計 7ms。ホームの体感が落ちるか 10万件を超えたら、直近N件のカーソル読みに切り替える。
