...
- **Svelte / Vue**: バンドルサイズや学習コストで利点はあるが、情報量・AI エージェントの習熟度・オーナーの選好で React。
  - 2026-07-11 追記: CI 構築後にオーナーの「短いコードで書けるほうがよい」という観点で Svelte 5 への乗り換えを再検討(同一コンポーネントのコード比較を提示)。コードの短さ・依存削減では Svelte 優位だが、情報量・AI 習熟度を優先して **React 継続を確定**。UI コードは React の範囲で簡潔に保つ(小さなコンポーネント・早すぎる抽象化をしない)。
