...
/**
 * Push the export JSON to the gist (creating it on first call). Returns the
 * updated config (with gistId + timestamp) so callers can persist it.
 * @throws Error on network/auth failure.
 */
export async function pushBackup(json: string): Promise<BackupConfig> {
  ...
  const data = (await githubJson(
    config.gistId ? `${API}/${config.gistId}` : API,
    config.token,
    { method: config.gistId ? "PATCH" : "POST", body },
  )) as { id: string };
...
export async function pullBackup(): Promise<string | null> {
  ...
  const data = (await githubJson(`${API}/${config.gistId}`, config.token)) as {
